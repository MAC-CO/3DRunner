...
class ScriptUsageProgrammerSounds : MonoBehaviour
{
    ...
    void Reset()
    {
        EventName = FMODUnity.EventReference.Find("event:/Dialogos");
    }
#endif
    ...
    public void PlayDialogue(string key)
    {
        ...
        {
            // Si ya hay un evento sonando, no hacemos nada
            return;
        }

        currentDialogueInstance = FMODUnity.RuntimeManager.CreateInstance(EventName);
    ...
    // public void buttonPlayDialog(string Code)
    // {
    //     PlayDialogue("ESPANO_01");
    // }
    ...
    {
        return currentDialogueInstance.isValid() && currentDialogueInstance.isValid();
    }
}
